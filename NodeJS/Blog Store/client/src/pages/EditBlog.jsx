...
const EditBlog = () => {
  ...
  useEffect(() => {
    ...
    const getBlog = async () => {
      ...
      try {
        ...
        // Admins or original author can edit
        if (data.author?._id !== user?._id && user?.role !== 'Administrator') {
          toast.error('Unauthorized timeline edit request.');
          navigate('/');
          return;
        }
        ...
        toast.error('Failed to retrieve manuscript coordinates.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    const toastId = toast.loading('Re-archiving manuscript updates...');
    try {
      await updateBlog(id, { title, category, content, coverImage });
      toast.success('Manuscript successfully re-archived!', { id: toastId });
      ...
      toast.error(err.response?.data?.message || 'Failed to update manuscript', { id: toastId });
    }
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center text-center p-4">
        ...
        <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] italic">De-serializing Manuscript Coordinates...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 pt-36 pb-20 px-4">
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        {/* Navigation link */}
        <Link 
          to={user.role === 'Administrator' ? '/admin' : '/creator'} 
          className="inline-flex items-center gap-2 text-slate-400 hover:text-blue-600 transition-all font-black uppercase tracking-[0.3em] text-[10px] italic group"
        >
          <span className="group-hover:-translate-x-2 transition-transform">←</span> Return to Dashboard
        </Link>
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-white rounded-[48px] overflow-hidden border border-slate-100/50 shadow-2xl">
              ...
              <div className="bg-slate-900 p-10 text-white relative overflow-hidden">
                ...
                <h1 className="text-4xl font-black italic tracking-tighter relative z-10">Edit Manuscript</h1>
                <p className="text-slate-400 mt-2 text-sm font-medium italic relative z-10">Curate and optimize manuscript records already embedded in timelines.</p>
              </div>
